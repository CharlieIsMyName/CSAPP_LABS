...
void *mm_realloc(void *ptr, size_t size){
    ...
    void *ans=mm_malloc(size);
    if(GETSIZE(HDP(ptr))<=GETSIZE(HDP(ans))){
        memcpy(ans,ptr,GETSIZE(HDP(ptr))-4);
    }
    else{
        memcpy(ans,ptr,GETSIZE(HDP(ans))-4);
    }
    mm_free(ptr);
    return ans;
}
void *extendHeap(unsigned size){   //extend the heap by size, add to the list, and return the block pointer
    assert(size>=16);
...
void *coalesce(void *p){
    unsigned prevAlloc=GETALLOC(HDP(p))&3,nextAlloc=GETALLOC(HDP(NXBLKP(p)))&3;
